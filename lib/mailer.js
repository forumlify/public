...
function describeSmtpError(error) {
  ...
  // Resend 会拒绝发往 example.com、test.com 等保留/测试域名，
  // 以及未验证域名下的地址。原始报文是英文且较长，这里转成中文。
  if (/Invalid `?to`? field/i.test(message) || /testing email address/i.test(message)) {
    return '收件人邮箱被邮件服务商拒绝：请使用真实可收信的邮箱地址。'
      + 'Resend 不允许发送到 example.com、test.com 等测试域名，'
      + '也不允许从尚未验证的域名发信';
  }
  if (/domain is not verified|not verified/i.test(message)) {
    return '发信域名尚未验证：请先在邮件服务商处完成发件域名的验证（配置 SPF / DKIM 记录）';
  }
  if (/API key is invalid|invalid.*api.*key/i.test(message)) {
    return 'API Key 无效：请检查后台填写的密钥是否正确、是否已被撤销';
  }
  if (/rate limit|too many requests/i.test(message)) {
    return '发送频率超出邮件服务商限制：请稍后再试，或升级服务商配额';
  }
